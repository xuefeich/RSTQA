...
class TagopFineTuningModel():
    def __init__(self, args, network,engine, state_dict=None, num_train_steps=1):
        self.args = args
        self.train_loss = AverageMeter()
        self.dev_loss = AverageMeter()
        self.step = 0
    ...
    def update(self, tasks,epoch):
        for key in tasks:
            try:
                tasks[key] = tasks[key].to(model_engine.local_rank)
            except:
                continue
        self.network.train()
        output_dict = self.engine(**tasks,epoch = epoch)
        loss = output_dict["loss"]
