...
class TagopModel(nn.Module):
    def __init__(self,
        ...
        self.span_predictor = FFNLayer(hidden_size,hidden_size,  2, dropout_prob)
        ...
        #self.opt_predictor = BiFFNLayer(hidden_size,hidden_size, hidden_size, 2, dropout_prob)
        # order predictor: three-class classification
        self.order_predictor = FFNLayer(3*hidden_size, hidden_size, 2, dropout_prob)
        #self.order_predictor = BiFFNLayer(2*hidden_size,hidden_size, hidden_size, 2, dropout_prob)
    ...
    def forward(self,
        ...
        table_tag_labels = util.replace_masked_values(tag_labels.float(), table_mask.unsqueeze(1).expand(batch_size,self.num_ops,inlen), -100)

        paragraph_sequence_output  = util.replace_masked_values(sequence_output, paragraph_mask.unsqueeze(-1), 0)
        paragraph_tag_labels = util.replace_masked_values(tag_labels.float(), paragraph_mask.unsqueeze(1).expand(batch_size,self.num_ops,inlen), -100)
        ...
        for bsz in range(batch_size):
            ...
            for roud in range(self.num_ops):
                ...
                if ari_ops[bsz,roud] != -100:
                    opt = sequence_output[bsz,opt_mask[bsz]+roud].expand(inlen,self.hidden_size)
                    opt_t = util.replace_masked_values(opt, table_mask[bsz].unsqueeze(-1), 0)
                    opt_p = util.replace_masked_values(opt, paragraph_mask[bsz].unsqueeze(-1), 0)
                    tout = torch.cat((table_sequence_output[bsz] , opt_t) , dim = -1)
                    pout = torch.cat((paragraph_sequence_output[bsz] , opt_p) , dim = -1)
                    output_dict["loss"] = output_dict["loss"] + self.NLLLoss(util.masked_log_softmax(self.tag_predictor(tout), mask=None),table_tag_labels[bsz,roud].long())
                    output_dict["loss"] = output_dict["loss"] + self.NLLLoss(util.masked_log_softmax(self.tag_predictor(pout), mask=None),paragraph_tag_labels[bsz,roud].long())
                elif roud == 0:
                    output_dict["loss"] = output_dict["loss"] + self.NLLLoss(util.masked_log_softmax(self.span_predictor(table_sequence_output[bsz]), mask=None),table_tag_labels[bsz,roud].long())
                    output_dict["loss"] = output_dict["loss"] + self.NLLLoss(util.masked_log_softmax(self.span_predictor(paragraph_sequence_output[bsz]), mask=None),paragraph_tag_labels[bsz,roud].long())



        #if len(torch.nonzero(ari_ops != -100)) == 0:
        #    ari_ops_loss = torch.tensor(0,device = device)
        #else:
        #    ari_ops_loss = self.ari_operator_criterion(ari_ops_prediction.transpose(1,2) , ari_ops)

        #modify the input_ids
        #input_ids_filled = replace_opt(input_ids,opt_mask,self.ari_operator_ids,ari_ops,batch_size,self.num_ops)
            ...
            for bsz in range(batch_size):
               for roud in range(self.num_ops):
                  if order_labels[bsz,roud] != -100:
                     opd1_output = torch.mean(sequence_output[bsz , selected_indexes[bsz][roud][0]],dim = 0)
                     opd2_output = torch.mean(sequence_output[bsz , selected_indexes[bsz][roud][1]],dim = 0)
    ...
    def predict(self,
        ...
        table_cell_tag_prediction = torch.zeros([batch_size,self.num_ops,sequence_output.shape[1]],device = device)
        paragraph_token_tag_prediction =  torch.zeros([batch_size,self.num_ops,sequence_output.shape[1]],device = device)
        table_cell_tag_prediction_score = torch.zeros([batch_size,self.num_ops,sequence_output.shape[1]],device = device)
        paragraph_token_tag_prediction_score =  torch.zeros([batch_size,self.num_ops,sequence_output.shape[1]],device = device)
        ...
        for bsz in range(batch_size):
            for roud in range(self.num_ops):
                if pred_ari_class[bsz,roud] != 0:
                    opt = sequence_output[bsz,opt_mask[bsz]+roud].expand(inlen,self.hidden_size)
                    opt_t = util.replace_masked_values(opt, table_mask[bsz].unsqueeze(-1), 0)
                    opt_p = util.replace_masked_values(opt, paragraph_mask[bsz].unsqueeze(-1), 0)
                    tout = torch.cat((table_sequence_output[bsz] , opt_t) , dim = -1)
                    pout = torch.cat((paragraph_sequence_output[bsz] , opt_p) , dim = -1)
                    table_tag_prediction = util.replace_masked_values(util.masked_log_softmax(self.tag_predictor(tout), mask=None),table_mask[bsz].unsqueeze(-1), 0)
                    paragraph_tag_prediction = util.replace_masked_values(util.masked_log_softmax(self.tag_predictor(pout), mask=None),paragraph_mask[bsz].unsqueeze(-1), 0)
                    paragraph_tag_prediction_score = paragraph_tag_prediction[ :, 1]
                    #paragraph_token_tag_prediction_score[bsz,roud] = _index_reduce_max_bsz(paragraph_tag_prediction_score, paragraph_index[bsz],max_length=512, name="index_reduce_max")
                    paragraph_token_tag_prediction_score[bsz,roud] = paragraph_tag_prediction_score
                    paragraph_tag_prediction_argmax = torch.argmax(paragraph_tag_prediction, dim=-1).float()
                    paragraph_token_tag_prediction[bsz,roud] = paragraph_tag_prediction_argmax
                    table_tag_prediction_score = table_tag_prediction[ :, 1]
                    table_cell_tag_prediction_score[bsz,roud] = table_tag_prediction_score
                    table_tag_prediction_argmax = torch.argmax(table_tag_prediction, dim=-1).float()
                    table_cell_tag_prediction[bsz,roud] = table_tag_prediction_argmax
                elif roud == 0:
                    table_tag_prediction = util.replace_masked_values(util.masked_log_softmax(self.span_predictor(table_sequence_output[bsz]), mask=None),table_mask[bsz].unsqueeze(-1), 0)
                    paragraph_tag_prediction = util.replace_masked_values(util.masked_log_softmax(self.span_predictor(paragraph_sequence_output[bsz]), mask=None),paragraph_mask[bsz].unsqueeze(-1), 0)
                    paragraph_tag_prediction_score = paragraph_tag_prediction[ :, 1]
                    #paragraph_token_tag_prediction_score[bsz,roud] = _index_reduce_max_bsz(paragraph_tag_prediction_score, paragraph_index[bsz],max_length=512, name="index_reduce_max")
                    paragraph_token_tag_prediction_score[bsz,roud] = paragraph_tag_prediction_score
                    paragraph_tag_prediction_argmax = torch.argmax(paragraph_tag_prediction, dim=-1).float()
                    ...
                    '''
                    table_tag_reduce_max_prediction, _ = reduce_max_index_get_vector(table_tag_prediction[ :, 1], tout, table_cell_index)
                    paragraph_tag_reduce_max_prediction, _ = reduce_max_index_get_vector(paragraph_tag_prediction[ :, 1], pout, paragraph_index)
                    ...
                    masked_paragraph_tag_reduce_max_prediction = util.replace_masked_values(paragraph_tag_reduce_max_prediction,
                                                                                ...
                                                                                -1e+5)
                    sorted_table_tag_prediction, sorted_cell_index = torch.sort(masked_table_tag_reduce_max_prediction,dim=-1, descending=True)
        ...
        for roud in range(self.num_ops):
            paragraph_token_tag_prediction_score[:,roud,:] = reduce_max_index(paragraph_token_tag_prediction_score[:,roud,:], paragraph_index)
            paragraph_token_tag_prediction[:,roud,:] = reduce_mean_index(paragraph_token_tag_prediction[:,roud,:],paragraph_index)
            table_cell_tag_prediction_score[:,roud,:] = reduce_max_index(table_cell_tag_prediction_score[:,roud,:],table_cell_index)
            table_cell_tag_prediction[:,roud,:] = reduce_mean_index(table_cell_tag_prediction[:,roud,:],table_cell_index)
        '''
        paragraph_index_rd = paragraph_index.unsqueeze(1).expand(batch_size,self.num_ops,inlen)
        table_cell_index_rd = table_cell_index.unsqueeze(1).expand(batch_size,self.num_ops,inlen)
        paragraph_token_tag_prediction_score = reduce_max_index(paragraph_token_tag_prediction_score, paragraph_index_rd)
        paragraph_token_tag_prediction = reduce_mean_index(paragraph_token_tag_prediction,paragraph_index_rd).detach().cpu().numpy()
        table_cell_tag_prediction_score = reduce_max_index(table_cell_tag_prediction_score,table_cell_index_rd)
        table_cell_tag_prediction = reduce_mean_index(table_cell_tag_prediction,table_cell_index_rd).detach().cpu().numpy()
        '''
        ...
        for bsz in range(batch_size):
            ...
            #number_indexes_batch.append([])
            for roud in range(self.num_ops):
                para_sel_indexes , paragraph_selected_numbers = get_number_index_from_reduce_sequence(paragraph_token_tag_prediction[bsz,roud],paragraph_numbers[bsz])
                table_sel_indexes , table_selected_numbers = get_number_index_from_reduce_sequence(table_cell_tag_prediction[bsz,roud], table_cell_numbers[bsz])
                ...
                if not selected_numbers:
                    selected_numbers_batch[bsz].append([])
                    #number_indexes_batch[bsz].append([])
                else:
                    pn = len(para_sel_indexes)
                    tn = len(table_sel_indexes)
                    k = 0
                    selected_numbers_batch[bsz].append(selected_numbers)
                    #number_indexes_batch[bsz].append(selected_indexes)
                    ...
                    for sel_index in selected_indexes:
                        if k < pn:
                            ...
                            cur_score = torch.mean(paragraph_token_tag_prediction_score[bsz,roud,selected_index])
                        else:
                            selected_index = torch.nonzero(table_cell_index[bsz] == sel_index).squeeze(-1)
                            cur_score = torch.mean(table_cell_tag_prediction_score[bsz,roud,selected_index])
        ...
        for bsz in range(batch_size):
            ...
            if "SPAN-TEXT" in self.OPERATOR_CLASSES and predicted_operator_class[bsz] == self.OPERATOR_CLASSES["SPAN-TEXT"]:
                paragraph_selected_span_tokens = get_single_span_tokens_from_paragraph(
                      paragraph_token_tag_prediction[bsz,0],
                      paragraph_token_tag_prediction_score[bsz,0],
                ...
                table_selected_tokens = get_single_span_tokens_from_table(
                   table_cell_tag_prediction[bsz,0],
                   table_cell_tag_prediction_score[bsz,0],
                ...
                paragraph_selected_span_tokens = get_span_tokens_from_paragraph(paragraph_token_tag_prediction[bsz,0], paragraph_tokens[bsz])
                table_selected_tokens = get_span_tokens_from_table(table_cell_tag_prediction[bsz,0], table_cell_tokens[bsz])
                ...
                paragraph_selected_tokens = \
                    get_span_tokens_from_paragraph(paragraph_token_tag_prediction[bsz,0], paragraph_tokens[bsz])
                table_selected_tokens = \
                    get_span_tokens_from_table(table_cell_tag_prediction[bsz,0], table_cell_tokens[bsz])
                        ...
                        for roud in range(self.num_ops):
                            if "STP" in self.ARI_CLASSES and pred_ari_class[bsz,roud] == self.ARI_CLASSES["STP"]:
                                if roud == 0:
                                    ...
                                    current_ops[roud] = "Stop"
                                break
                            roud_selected_numbers = selected_numbers_batch[bsz][roud]
                            if roud > 0 :
...
def _index_reduce_bsz(values, index, max_length, index_reduce_fn, name):
    flat_index, num_index = flatten_index(index, max_length)
    seq_len = values.shape[0]
    flat_values = values.reshape(1 * seq_len)
    index_means = scatter(
        src=flat_values,
        index=flat_index.type(torch.long),
        dim=0,
    ...
    output_values = index_means.view(1, -1)
    return output_values
...
def _index_reduce_max_bsz(values, index, max_length, name):
    flat_index, num_index = flatten_index(index, max_length)
    seq_len = values.shape[0]
    flat_values = values.reshape(1*seq_len)
    index_max, _ = scatter_max(
    ...
    output_values = index_max.view(1, -1)
    return output_values
