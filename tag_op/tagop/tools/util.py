...
class BiFFNLayer(nn.Module):
    def __init__(self, input_dim1,input_dim2, intermediate_dim, output_dim, dropout, layer_norm=True):
        super(BiFFNLayer, self).__init__()
        self.fc1 = nn.Bilinear(input_dim1,input_dim2, intermediate_dim)
        if layer_norm:
            self.ln = nn.LayerNorm(intermediate_dim)
        else:
            self.ln = None
    ...
    def forward(self, input1,input2):
        inter = self.fc1(self.dropout_func(input1),self.dropout_func(input2))
        inter_act = gelu(inter)
        if self.ln:
            inter_act = self.ln(inter_act)
        return self.fc2(inter_act)
