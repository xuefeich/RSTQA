...
class TagopModel(nn.Module):
    def __init__(self,
        ...
        # scale predictor
        self.scale_predictor = BiFFNLayer(hidden_size,2*hidden_size, hidden_size, scale_classes, dropout_prob)
        #self.round_predictor = FFNLayer(3 * hidden_size, hidden_size, scale_classes, dropout_prob)
        # tag predictor: two-class classification
        self.tag_predictor = BiFFNLayer(hidden_size,hidden_size, hidden_size, 2, dropout_prob)
    ...
    def forward(self,
        ...
        table_tag_prediction = self.tag_predictor(table_sequence_output,table_cls_output)
        ...
        paragraph_tag_prediction = self.tag_predictor(paragraph_sequence_output,paragraph_cls_output)
        ...
        scale_output = torch.cat((table_reduce_mean, paragraph_reduce_mean), dim=-1)
        operator_prediction = self.operator_predictor(cls_output)

        scale_prediction = self.scale_predictor(question_reduce_mean,scale_output)
        output_dict = {}
        ...
        if len(torch.nonzero(order_labels == -100)) < batch_size * self.num_ops:
            order_output = torch.zeros([batch_size,self.num_ops,2*self.hidden_size],device = device)
            for bsz in range(batch_size):
               for roud in range(self.num_ops):
                  if order_labels[bsz,roud] != -100:
                     ...
                     order_output[bsz,roud] = torch.cat((opd1_output , opd2_output),dim = -1)
            ...
            order_prediction = self.order_predictor(order_output,opt_output_filled)
    ...
    def predict(self,
            ...
            if not selected_numbers:
                ...
                for roud in range(self.num_ops):
                    ...
                    if top_2_indexes[bsz,roud,0] != 0 and top_2_indexes[bsz,roud,1] != 0:
                        ...
                        pred_order[bsz,roud] = torch.argmax(self.order_predictor(torch.cat((sequence_output[bsz , int(first_index)],  sequence_output[bsz , int(sec_index)]),dim=-1),opt_ouput[bsz,roud]),dim = -1)
        ...
        if num_numbers > 0:
            ...
            for i in range(1,self.num_ops):
                for j in range(i):
                    #ari_opt_prediction = self.opt_predictor(opt_output[:,j,:],opt_output[:,i,:])
