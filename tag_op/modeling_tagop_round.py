...
class TagopModel(nn.Module):
    def __init__(self,
        ...
        self.ari_predictor = FFNLayer(2*hidden_size, hidden_size, ari_classes, dropout_prob)
        # scale predictor
    ...
    def forward(self,
        ...
        round_loss = self.round_criterion(round_prediction, round_labels)
        ...
        output_dict["loss"] = operator_prediction_loss + scale_prediction_loss + table_tag_prediction_loss + paragraph_token_tag_prediction_loss+round_loss


        pred_ari_class = torch.zeros([batch_size,self.num_ops],device = device)

        for bsz in range(batch_size):
            for roud in range(round_labels[bsz]):
                output_dict["loss"] = output_dict["loss"] + self.ari_operator_criterion(self.ari_predictor(torch.cat((sequence_output[bsz,opt_mask[bsz]+roud],question_reduce_mean),dim = -1)).unsqueeze(0) , ari_ops[bsz,roud].unsqueeze(0))
    ...
    def predict(self,
        ...
        #opt_output = sequence_output.gather(index = opt_index,dim = 1)
        opt_output = torch.zeros([batch_size,self.num_ops,self.hidden_size],device = device)
        ops_output = torch.zeros([batch_size,self.num_ops,2*self.hidden_size],device = device)
        for bsz in range(batch_size):
            opt_output[bsz] = sequence_output[bsz,opt_mask[bsz]:opt_mask[bsz]+self.num_ops,:]
            for roud in range(self.num_ops):
                ops_output[bsz,roud] = torch.cat((opt_output[bsz,roud],question_reduce_mean),dim = -1)


        ari_ops_prediction = self.ari_predictor(ops_output)
        pred_ari_class = torch.argmax(ari_ops_prediction,dim = -1)
            ...
            if "SPAN-TEXT" in self.OPERATOR_CLASSES and predicted_operator_class[bsz] == self.OPERATOR_CLASSES["SPAN-TEXT"]:
                ...
                if num_numbers == 0:
                    ...
                    if len(selected_numbers) == 0:
                        ...
                        temp_ans = []
                        for roud in range(self.num_ops):
                            if roud == int(predicted_round_class[bsz])+1:
                                if roud == 0:
                                    answer = ""
                            ...
                            if len(roud_selected_numbers) == 0 and roud != 0:
                                ...
                                if "SUM" in self.ARI_CLASSES and pred_ari_class[bsz,roud] == self.ARI_CLASSES["SUM"]:
                                    temp_ans.append(np.sum(selected_numbers))
