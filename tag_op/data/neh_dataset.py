    ...
    for i in range(len(table)):
        for j in range(len(table[i])):
            ...
            if is_number(table[i][j]):
                ...
                table_ids_numbers += [to_number(table[i][j])]*len(cell_ids)
            else:
                table_cell_number_value.append(np.nan)
                table_ids_numbers += np.nan*len(cell_ids)
            ...
            current_cell_index += 1
    return table_cell_tokens, table_ids, table_tags, table_cell_number_value, table_cell_index,table_ids_numbers
