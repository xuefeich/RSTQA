...
ARITHMETIC_CLASSES_ = {"STP":0,"SUM":1,"DIFF":2,"TIMES":3,"DIVIDE":4,"AVERAGE":5}
...
def _is_average(num_facts:list, answer):
    ans = round(answer,2)
    if round(np.average(num_facts), 2) == ans:
        return True
    else:
        for i in range(len(num_facts)):
            num_facts[i] *= -1
            if round(np.average(num_facts)) == ans:
                return True
            else:
                num_facts[i] *= -1
    return False
