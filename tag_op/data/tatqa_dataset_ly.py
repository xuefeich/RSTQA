...
class TagTaTQATestReader(object):
    ...
    def summerize_op(self, derivation, answer_type, facts, answer, answer_mapping, scale,tv,pv):
        ...
        if answer_type == "span":
                           ...
                           for num in ari[1:]:
                               if not isinstance(num,str):
                                   if num not in truth_numbers:
                                       ...
                                       truth_operands[num]=[i]
                               else:
                                 truth_operands[num].append(i)
            ...
            return operator_classes,truth_numbers,order_labels,truth_operands
    ...
    def _to_test_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
        ...
        if self.mode == "dev":
            gold_ops,truth_numbers,order_labels,truth_operands = self.summerize_op(derivation, answer_type, facts, answer, answer_mapping, scale,table_cell_number_value,paragraph_number_value)
            if gold_ops is None:
        ...
        answer_dict = {"answer_type": answer_type, "answer": answer, "scale": scale, "answer_from": answer_from,
                "gold_ops": gold_ops, "gold_scale":scale,"order_labels":order_labels,"truth_numbers":truth_numbers,"truth_operands":truth_operands}
