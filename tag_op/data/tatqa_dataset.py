...
def _concat(question_ids,
    ...
    if ari_tags != None:
      ...
      for i in range(num_ops):
         ...
         if isinstance(ari_table_tags[i],dict):
             ...
             for j in np.where(opd2_tags == 1)[0]:
                 opd1_tags[j] = 2
             ...
             if paragraph_length > 1:
                ...
                for j in np.where(p_opd2_tags == 1)[0]:
                    p_opd1_tags[j] = 2
    ...
    return input_ids, attention_mask, paragraph_mask, paragraph_number_value, paragraph_index, \
           table_mask, table_cell_number_value, table_index, tags, input_segments,opt_mask,opt_index,ari_round_tags,opd_two_tags,ari_round_labels
...
def _test_concat(question_ids,
    ...
    del in_paragraph_index
    return input_ids, attention_mask, paragraph_mask, paragraph_number_value, paragraph_index, \
           table_mask, table_cell_number_value, table_index, tags, input_segments,opt_mask,opt_index
...
class TagTaTQAReader(object):
    ...
    def _make_instance(self, input_ids, attention_mask, token_type_ids, paragraph_mask, table_mask,
                       paragraph_number_value, table_cell_number_value, paragraph_index, table_cell_index,
                        ...
                        table_cell_tokens, answer_dict, question_id, ari_ops,opt_mask,opt_index,opt_labels,ari_labels):
        ...
        if ari_ops == [-100] * self.num_ops:
            ...
            for i in range(1,self.num_ops-1):
                ...
                if ari_ops[i] == -100:
                    ...
                    #opt_labels[0,i:,:] = -100
                    #opt_labels[0,:,i:] = -100
        #if ari_ops != [-100] * self.num_ops and ari_ops[1] != 0:
            #print(len(torch.nonzero(ari_labels == 0)))
            #print("--------------------------")
        #   print(opt_labels)
        #   print("----------------------------------------")
        #if ari_ops == [-100] * self.num_ops:
        #    print(torch.nonzero(ari_labels == 0))

        #ari_operators = torch.zeros([1,self.num_ops])
        #ari_operators[0] = torch.LongTensor(ari_ops)
        opt_id = torch.nonzero(opt_mask == 1)[0,1]
        #opt_indexes = torch.zeros([1,self.num_ops,1024])
        #for i in range(self.num_ops):ssaaa
        #    j = opt_id + i
        #    opt_indexes[0,i,:] = j
        #print(torch.nonzero(opt_mask == 1))
        #exit(0)
        return {
            ...
            "ari_labels" : torch.LongTensor(np.array(ari_labels)),
    ...
    def _to_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
                       ...
                       if temp_mapping == None:
                           ...
                           if ari_operations[i][0] in ['DIFF','DIVIDE']:
                              if "paragraph" in operand_one_mapping:
                                  print(operand_one_mapping["paragraph"])
                              if "paragraph" in operand_two_mapping:
                                  print(operand_two_mapping["paragraph"])
        ...
        table_mask, table_number_value, table_index, tags, token_type_ids , opt_mask,opt_index,ari_round_tags,opd_two_tags,ari_round_labels = \
        ...
        return self._make_instance(input_ids, attention_mask, token_type_ids, paragraph_mask, table_mask,
                    paragraph_number_value, table_number_value, paragraph_index, table_index, tags, operator_class, scale_class,
                    paragraph_tokens, table_cell_tokens, answer_dict, question_id,ari_ops,opt_mask,opt_index,opt_labels,ari_round_labels)
...
class TagTaTQATestReader(object):
    def __init__(self, tokenizer,
                 ...
                 ablation_mode=0, op_mode=0,num_ari_ops=6):
        self.max_pieces = 512
        self.tokenizer = tokenizer
        self.passage_length_limit = passage_length_limit
        self.question_length_limit = question_length_limit
    ...
    def _make_instance(self, input_ids, attention_mask, token_type_ids, paragraph_mask, table_mask,
                        paragraph_number_value, table_cell_number_value, paragraph_index, table_cell_index,
                        tags_ground_truth, paragraph_tokens, table_cell_tokens, answer_dict, question_id,opt_mask,opt_index):
    ...
    def summerize_op(self, derivation, answer_type, facts, answer, answer_mapping, scale):
        if answer_type == "span":
            if "table" in answer_mapping.keys():
                ...
                return ["Cell-in-table"] +["ignore"]* (self.num_ops-1)
            elif "paragraph" in answer_mapping.keys():
                self.op_count["Span-in-text"] += 1
                return ["Span-in-text"] +["ignore"]* (self.num_ops-1)
        elif answer_type == "multi-span":
            self.op_count["Spans"] += 1
            return ["Spans"]  +["ignore"]* (self.num_ops-1)
        elif answer_type == "count":
            self.op_count["Count"] += 1
            return ["Count"] +["ignore"] * (self.num_ops-1)
    ...
    def _to_test_instance(self, question: str, table: List[List[str]], paragraphs: List[Dict], answer_from: str,
        ...
        gold_ops = self.summerize_op(derivation, answer_type, facts, answer, answer_mapping, scale)
        if gold_ops is None:
        ...
        input_ids, attention_mask, paragraph_mask, paragraph_number_value, paragraph_index, \
        table_mask, table_number_value, table_index, tags, token_type_ids ,opt_mask,opt_index= \
