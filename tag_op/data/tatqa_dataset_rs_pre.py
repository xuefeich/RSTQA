...
from .data_util_round import *
from .data_util_round import  _is_average, _is_change_ratio, _is_diff, _is_division, _is_sum, _is_times
                ...
                if ari_ops[i] in [1,3]:
                    ...
                    for sl in ari_sel_labels:
                        if sl[i] == 1:
                            count += 1
                    if count == 1:
                        if 1 in opt_labels[0,:,i-1]:
                            order_labels[i] = 1
                        elif 2 in opt_labels[0,:,i-1]:
                            for j in range(self.num_ops - 1):
                                if opt_labels[0,j,i-1] == 2:
                                    opt_labels[0,j,i-1] = 1
                        else:
                            print("err1 for diff/divide")
                            return None
                    elif count == 0:
                        opt1 = None
                        opt2 = None
                        for j in range(self.num_ops - 1):
                            if opt_labels[0,j,i-1] == 1:
                                opt1 = j
                            elif opt_labels[0,j,i-1] == 2:
                                opt_labels[0,j,i-1] = 1
                                opt2 = j
                        ...
                        print(question_id)
                        print(table_cell_index[0,64])
                        print(table_cell_index[0,65])
                        print(number_indexes)
                        print(ari_sel_labels)
                        print(i)
                        print("err3 for diff/divide")
                        exit(0)
                        #order_labels[i] = -100
                else:
                    order_labels[i] = -100
    ...
    def summerize_op(self, derivation, answer_type, facts, answer, answer_mapping, scale):
        ...
        if answer_type == "span":
            if "table" in answer_mapping.keys():
                ...
                return ["Cell-in-table"] +["ignore"]* (self.num_ops-1),truth_numbers
            elif "paragraph" in answer_mapping.keys():
                self.op_count["Span-in-text"] += 1
                return ["Span-in-text"] +["ignore"]* (self.num_ops-1) , truth_numbers
        elif answer_type == "multi-span":
            self.op_count["Spans"] += 1
            return ["Spans"]  +["ignore"]* (self.num_ops-1),truth_numbers
        elif answer_type == "count":
            self.op_count["Count"] += 1
            return ["Count"] +["ignore"] * (self.num_ops-1),truth_numbers
            ...
            return operator_classes,truth_numbers
        ...
        for one in tqdm(dataset):
            ...
            for question_answer in questions:
                ...
                if instance is not None:
                    ...
                    if instance["answer_dict"]["gold_ops"][2] == "Stop":
