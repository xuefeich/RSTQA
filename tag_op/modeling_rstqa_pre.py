...
from tatqa_metric_rs import TaTQAEmAndF1
from .tools.util import FFNLayer
from .tools import allennlp as util
from typing import Dict, List, Tuple
import numpy as np
from tag_op.data.file_utils import is_scatter_available
from tag_op.data.data_util_round import get_op_1, get_op_2, get_op_3, SCALE, OPERATOR_CLASSES_,ARITHMETIC_CLASSES_
...
class RstqaModel(nn.Module):
    def __init__(self,
                 ...
                 round_criterion: nn.CrossEntropyLoss = None,
        ...
        super(RstqaModel, self).__init__()
        ...
        self.round_predictor = FFNLayer(2*hidden_size, hidden_size, self.num_ops, dropout_prob)

        self.opt_predictor = FFNLayer(2*hidden_size, hidden_size, 2, dropout_prob)
        ...
        self.round_criterion = round_criterion
    ...
    def forward(self,
        ...
        if len(torch.nonzero(round_labels == -100)) < batch_size:
            round_prediction = self.round_predictor(torch.cat((cls_output , question_reduce_mean),dim = -1))
            output_dict["loss"] = output_dict["loss"] + self.round_criterion(round_prediction, round_labels)
               ...
               for selected_index in selected_indexes:
                   if selected_index[0] == bsz:
                       ...
                       for roud in range(self.num_ops):
                           ...
                           #exit(0)
                           #selected_numbers_output[num_numbers,roud] = torch.cat((torch.mean(sequence_output_filled[bsz , number_index],dim = 0), opt_output_filled[bsz,roud]),dim = -1)
            ...
            for bsz in range(batch_size):
               for roud in range(self.num_ops):
                  if order_labels[bsz,roud] != -100:
                     ...
                     if roud >0 :
                         for j in range(roud):
                             if opt_labels[bsz,j, roud - 1] == 1:
                                 if opt1 == -100:
                                     opt1 = j
                                 else:
                                     opt2 = j

                     if opt1 == -100 and opt2 == -100:
                         opd1_output = torch.mean(sequence_output_filled[bsz , order_numbers[bsz][roud][0]],dim = 0)
                         opd2_output = torch.mean(sequence_output_filled[bsz , order_numbers[bsz][roud][1]],dim = 0)
                     elif opt1 != -100 and opt2 == -100:
                         opd1_output = torch.mean(sequence_output_filled[bsz , order_numbers[bsz][roud][0]],dim = 0)
                         opd2_output = opt_output_filled[bsz,opt1]
                     else:
                         opd1_output = opt_output_filled[bsz,opt1]
                         opd2_output = opt_output_filled[bsz,opt2]
    ...
    def predict(self,
        ...
        round_prediction = self.round_predictor(torch.cat((cls_output,question_reduce_mean),dim = -1))
        predicted_round_class = torch.argmax(round_prediction, dim=-1)
        ...
        operand_prediction = torch.zeros([60,self.num_ops,2],device = device)
        scores = torch.zeros([batch_size,self.num_ops,2],device = device)
        ...
        for bsz in range(batch_size):
            ...
            if not selected_numbers:
                ...
                for roud in range(self.num_ops):
                    ...
                    if int(pred_ari_class[bsz,roud]) in [1,3]:
                        opt1 = -100
                        opt2 = -100
                        if roud >0 :
                            for j in range(roud):
                               if pred_opt_class[bsz,j, roud - 1] == 1:
                                   if opt1 == -100:
                                      opt1 = j
                                   else:
                                      opt2 = j

                        if opt1 == -100 and opt2 == -100 and top_2_indexes[bsz,roud,0] != 0 and top_2_indexes[bsz,roud,1] != 0:
                            opd1_output = sequence_output[bsz , int(first_index)]
                            opd2_output = sequence_output[bsz , int(sec_index)]
                            pred_order[bsz,roud] = torch.argmax(self.order_predictor(torch.cat((opd1_output, opt_output[bsz,roud] , opd2_output),dim=-1)),dim = -1)
                        elif opt1 != -100 and opt2 == -100 and top_indexes[bsz,roud] != 0:
                            opd1_output = sequence_output[bsz , int(top_indexes[bsz,roud])]
                            opd2_output = opt_output[bsz,opt1]
                            pred_order[bsz,roud] = torch.argmax(self.order_predictor(torch.cat((opd1_output, opt_output[bsz,roud] , opd2_output),dim=-1)),dim = -1)
                        elif opt1 != -100 and opt2 != -100:
                            opd1_output = opt_output[bsz,opt1]
                            opd2_output = opt_output[bsz,opt2]
                            pred_order[bsz,roud] = torch.argmax(self.order_predictor(torch.cat((opd1_output, opt_output[bsz,roud] , opd2_output),dim=-1)),dim = -1)
        ...
        for bsz in range(batch_size):
            '''
            if len(truth_numbers[bsz]) > 0:
                paragraph_selected_numbers = get_numbers_from_reduce_sequence(paragraph_token_tag_prediction[bsz],paragraph_numbers[bsz])
                table_selected_numbers = get_numbers_from_reduce_sequence(table_cell_tag_prediction[bsz], table_cell_numbers[bsz])
                selected_numbers = paragraph_selected_numbers + table_selected_numbers
                nc = 1
                if len(selected_numbers) != len(truth_numbers[bsz]):
                    nc = 0
                else:
                    for tru_number in truth_numbers[bsz]:
                        if tru_number not in selected_numbers:
                            nc = 0
                if gold_answers[bsz]["answer_from"] == "text":
                    self.text_numbers += 1
                    self.text_matched_numbers += nc
                elif gold_answers[bsz]["answer_from"] == "table":
                    self.table_numbers += 1
                    self.table_matched_numbers += nc
                else:
                    self.mix_numbers += 1
                    self.mix_matched_numbers += nc
            '''
            #print(gold_answers[bsz]["gold_ops"])
            #print(pred_ari_class[bsz])
            #print(selected_numbers_batch[bsz])
            ...
            if "SPAN-TEXT" in self.OPERATOR_CLASSES and predicted_operator_class[bsz] == self.OPERATOR_CLASSES["SPAN-TEXT"]:
                ...
                if num_numbers == 0:
                    ...
                    if len(selected_numbers) == 0:
                        ...
                        temp_ans = []
                        for roud in range(int(predicted_round_class[bsz])):
                            ...
                            if len(roud_selected_numbers) == 0 and roud != 0:
                                ...
                                if "SUM" in self.ARI_CLASSES and pred_ari_class[bsz,roud] == self.ARI_CLASSES["SUM"]:
                                    ...
                                    is_opt = False
                                    if roud > 0 :
                                        if len(opt_selected_numbers) == 0:
                                            operand_one = first_numbers[bsz,roud]
                                            operand_two = sec_numbers[bsz,roud]
                                        elif len(opt_selected_numbers) == 1:
                                            operand_one = top_numbers[bsz,roud]
                                            operand_two = opt_selected_numbers[0]
                                        elif len(opt_selected_numbers) == 2:
                                            operand_one = opt_selected_numbers[0]
                                            operand_two = opt_selected_numbers[1]
                                    else:
                                        operand_one = first_numbers[bsz,roud]
                                        operand_two = sec_numbers[bsz,roud]
