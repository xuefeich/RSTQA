...
from tag_op.data.hqa_batch_gen import TaTQATestBatchGen
from tag_op.data.hqa_data_util import OPERATOR_CLASSES_,ARITHMETIC_CLASSES_
from tag_op.data.hqa_data_util import get_op_1, get_op_2, get_arithmetic_op_index_1, get_arithmetic_op_index_2
...
def main():
    ...
    logger.info(f"Below are the result on {args.set} set...")
    model.reset()
    model.avg_reset()
    pred_json = model.predict(dev_itr)
    json.dump(pred_json,open(os.path.join(args.save_dir, f"hqa_result_on_{args.set}.json"), 'w'))
    model.get_metrics()
