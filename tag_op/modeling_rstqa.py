...
class RstqaModel(nn.Module):
    def __init__(self,
        ...
        # scale predictor
        self.scale_predictor = FFNLayer(4 * hidden_size, hidden_size, scale_classes, dropout_prob)
        self.round_predictor = FFNLayer(3 * hidden_size, hidden_size, self.num_ops, dropout_prob)
    ...
    def forward(self,
        ...
        scale_output = torch.cat((cls_output,question_reduce_mean, table_reduce_mean, paragraph_reduce_mean), dim=-1)
        ...
        if len(torch.nonzero(round_labels == -100)) < batch_size:
            round_prediction = self.round_predictor(torch.cat((cls_output , table_reduce_mean , paragraph_reduce_mean),dim = -1))
            output_dict["loss"] = output_dict["loss"] + self.round_criterion(round_prediction, round_labels)

        for bsz in range(batch_size):
            round_bsz  = int(round_labels[bsz])
            if round_bsz != -100:
                for roud in range(int(round_labels[bsz])+1):
    ...
    def predict(self,
        ...
        round_output = torch.cat(( cls_output, table_reduce_mean, paragraph_reduce_mean), dim=-1)
        round_prediction = self.round_predictor(round_output)
        scale_output = torch.cat(( cls_output, question_reduce_mean, table_reduce_mean, paragraph_reduce_mean), dim=-1)
        pred_round_class = torch.argmax(round_prediction, dim=-1)
            ...
            if "SPAN-TEXT" in self.OPERATOR_CLASSES and predicted_operator_class[bsz] == self.OPERATOR_CLASSES["SPAN-TEXT"]:
                ...
                if num_numbers == 0:
                    ...
                    if len(selected_numbers) == 0:
                        ...
                        temp_ans = []
                        for roud in range(int(pred_round_class[bsz]) + 1):
